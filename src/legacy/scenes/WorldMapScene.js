...
export default class WorldMapScene extends Phaser.Scene {
  ...
  create() {
    ...
    const video = this.add.video(0, 0, "road").setOrigin(0); // Начало координат в левом верхнем углу
  ...
  createPopup(text = "") {
    ...
    this.popupText = this.add
      .text(cameraCenterX, cameraCenterY - 50, text, {
        fontSize: "16px",
        fill: "#0f0",
      })
      .setOrigin(0.5);
  ...
  showPopup(hasNoButton) {
    // Генерируем врагов
    let [enemyName, enemiesToCreate] = this.getLevelConfig(
  ...
  handlePopupInput() {
    ...
    if (Phaser.Input.Keyboard.JustDown(this.spaceKey)) {
      if (this.selectedButton === "Yes") {
        ...
        if (
          ["Rat", "Mantis", "Cannibals", "Raiders"].includes(
            this.chosenEnemy.name
          )
        ) {
  ...
  getLevelConfig(player_level) {
    ...
    for (let i = 0; i < numberOfEnemies; i++) {
      const randomIndex = Math.floor(
        Math.random() * this.chosenEnemy.title.length
      );
