...
export class LSPSystem {
     ...
     */
    static async validateCollectionCompliance(baseClass, subClasses, constructorArgs) {
        const { LSPValidator } = await import('../validation/LSPValidators');
        const validator = new LSPValidator();
        for (const SubClass of subClasses) {
            const result = await validator.validateSubstitution(baseClass, SubClass, constructorArgs, []);
            if (!result.compliant) {
