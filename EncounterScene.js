...
export default class EncounterScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.add.graphics()
            ...
            .fillRect(100, 300, 824, 200);

        // Текстовые строки
        this.opponentText = this.add.text(512, 370, Phaser.Math.RND.pick([
        ...
        ]), { fontSize: '20px', fill: '#0f0' }).setOrigin(0.5);
        this.continueDialogText = this.add.text(512, 420, this.options[0], { fontSize: '16px', fill: '#0f0' }).setOrigin(0.5);
        this.attackText = this.add.text(512, 450, this.options[1], { fontSize: '16px', fill: '#0f0' }).setOrigin(0.5);
        this.leaveText = this.add.text(512, 480, this.options[2], { fontSize: '16px', fill: '#0f0' }).setOrigin(0.5);
    ...
    handleAskAboutThings() {
        ...
        if (Phaser.Math.Between(1, 100) <= speechSkill) {
            ...
            this.opponentText = this.add.text(512, 350, Phaser.Math.RND.pick([
            ...
            ]), { fontSize: '16px', fill: '#0f0' }).setOrigin(0.5);
            if (this.continueDialogText) this.continueDialogText.destroy();
            this.attackText.destroy();
            this.leaveText.destroy();
            this.attackText = this.add.text(512, 420, this.options[1], { fontSize: '16px', fill: '#0f0' }).setOrigin(0.5);
            this.leaveText = this.add.text(512, 450, this.options[2], { fontSize: '16px', fill: '#0f0' }).setOrigin(0.5);
            this.updateTextStyles();
        }
    }
    ...
    showResponse(response) {
        ...
        this.opponentText = this.add.text(512, 350, response, { fontSize: '16px', fill: '#0f0' }).setOrigin(0.5);

        // Обновляем варианты ответов героя
        if (response === this.problemText[0]) {
            this.options[0] = "I'll try to help. Show me where they are."
            if (this.continueDialogText) this.continueDialogText.destroy();
            this.continueDialogText = this.add.text(512, 420, this.options[0], { fontSize: '20px', fill: '#0f0' }).setOrigin(0.5);
            ...
            this.options[0] = 'Alright, I\'ll see what I can do to help with your agriculture.'
            if (this.continueDialogText) this.continueDialogText.destroy();
            this.continueDialogText = this.add.text(512, 420, this.options[0], { fontSize: '20px', fill: '#0f0' }).setOrigin(0.5);
        }
